...
namespace Fungus 
{
    ...
    {   
        ...
        private void Start()
        {
            ...
            agent = AdventureController.GetComponent<NavMeshAgent>();
        ...
        {
            while(Vector3.Distance(clickableObject.transform.position, AdventureController.transform.position) > clickableObject.activateDistance)
            {
                yield return new WaitForSeconds(0.1f);
            }

            if (Vector3.Distance(clickableObject.transform.position, AdventureController.transform.position) < clickableObject.activateDistance)
            {
                AdventureController.EnterDialogue();
                agent.SetDestination(AdventureController.transform.position);
                AdventureController.followSpot = AdventureController.transform.position;
